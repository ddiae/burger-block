...
// w-44 = 176px = same as tray block (w-52 container - p-4*2 padding)
const BASE = 'w-44 py-2 min-h-9 px-3 text-base font-black text-white flex items-center gap-1.5';
...
export default function BlockCanvas({
  chains, onChange, runningChainId, runningIndex = -1, disabled,
}: BlockCanvasProps) {
  ...
  const checkOverTrash = (curX: number, curY: number): boolean => {
    ...
    return gx < tr.right && gx + 144 > tr.left && gy < tr.bottom && gy + BLOCK_H > tr.top;
  };
  ...
  const onBlockPointerDown = (e: React.PointerEvent, chainId: string, fromIdx: number) => {
    ...
    // Capture on the CONTAINER so it survives block unmount during drag
