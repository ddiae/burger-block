...
import {
  BLOCK_LABELS,
  INGREDIENT_LABELS
} from "../../../entities/burger/data/burgers";
  ...
  return (
    <div className={`h-screen flex flex-col ${pageBg} overflow-hidden`}>
                ...
                <div className="flex-1 flex flex-col-reverse items-center justify-end overflow-hidden">
                  {animatedPlaced.map((type, i) => (
                    <div
                      ...
                      style={{
                        marginBottom: i === 0 ? 0 : type === "top_bun" ? 0 : -10
                      }}
                    >
      ...
      <div className="shrink-0 bg-white/70 backdrop-blur border-t border-white/50 px-4 py-3">
        <p className="text-base font-black text-gray-600 mb-2">
          🍔 나의 버거 리스트
        </p>
        {savedBurgers.length === 0 ? (
          ...
          <div className="flex gap-3 overflow-x-auto pb-1">
            {savedBurgers.map((b) => (
              <div
                key={b.id}
                className="shrink-0 bg-white rounded-2xl p-3 shadow-sm flex items-center gap-3 min-w-55"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 font-semibold">
                    {b.ingredients.length}가지 재료 · {b.createdAt}
                  </p>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {b.ingredients.slice(0, 3).map((ing, i) => (
                      <span
                        key={i}
                        className={`${BLOCK_COLORS[ing]} text-white text-xs font-black rounded px-1.5 py-0.5`}
                      >
                        {BLOCK_LABELS[ing]}
                      </span>
                    ))}
                ...
                <div className="flex flex-col gap-1 shrink-0">
                  <button
                    ...
                    className="bg-purple-100 hover:bg-purple-200 text-purple-700 font-black text-sm rounded-xl px-3 py-1.5 transition-all cursor-pointer"
                  >
                    ▶ 플레이
                  </button>
                  <button
                    onClick={() => handleDelete(b.id)}
                    className="bg-red-100 hover:bg-red-200 text-gray-100 hover:text-red-400 text-sm rounded-xl px-3 py-1.5 transition-all cursor-pointer"
                  >
                    삭제
                  </button>
                </div>
